...
// ── INTRO SPLASH VIDEO (homepage only, shown once per browser session) ──
(function(){
  const splash = document.getElementById('introSplash');
  if(!splash) return;

  const seen = sessionStorage.getItem('aman_intro_seen');
  const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if(seen || reduced){ splash.remove(); return; }

  const video       = splash.querySelector('video');
  const skipBtn     = splash.querySelector('.intro-skip');
  const unmuteBtn   = splash.querySelector('.intro-unmute');
  const progressBar = splash.querySelector('.intro-progress-bar');
  let closed = false;

  function closeSplash(){
    if(closed) return;
    closed = true;
    sessionStorage.setItem('aman_intro_seen', '1');
  ...
  skipBtn.addEventListener('keydown', (e) => { if(e.key==='Enter'||e.key===' '){e.preventDefault();closeSplash();} });

  unmuteBtn.addEventListener('click', () => {
    video.muted = !video.muted;
    unmuteBtn.textContent = video.muted ? '\u{1F507}' : '\u{1F50A}';
  ...
  // Safety net in case 'ended' never fires (stalled load, etc.)
  setTimeout(closeSplash, 16000);
})();
...
// ── ADMISSIONS PIPELINE (Google Sheet + Apps Script bridge) ──────────────
// Paste the "Web app" URL you get after deploying AMAN-Admissions-AppsScript.gs
...
function buildStudentHTML(n) {
  ...
  return `<div class="student-block" id="spanel${n}">
    ...
    <div class="frow">
      ...
      <div class="field"><label for="s${n}_program">Program Selection <span class="req">*</span></label>
    ...
    <div class="student-details" id="sdetails${n}" style="display:none;animation:slideDown 0.32s ease">
      ...
      <div class="frow dob-frow">
        <div class="field">
          <label for="s${n}_dob">Date of Birth <span class="req">*</span></label>
      ...
      <div class="frow">
        <div class="field"><label for="s${n}_gender">Gender <span class="req">*</span></label>
      ...
      <div class="frow">
        <div class="field"><label for="s${n}_grade">Grade</label>
          <select id="s${n}_grade"><option value="">Select grade...</option>${go}</select></div>
        <div class="field"><label for="s${n}_cost">Monthly Program Cost</label>
      ...
      <div class="frow full">
        <div class="field">
          <label for="s${n}_quran">Qur'an Reading Ability <span class="req">*</span></label>
      ...
      <div class="frow">
        <div class="field">
          <label for="s${n}_arabic">Arabic Language Level</label>
...
function submitForm() {
  ...
  const panels = document.querySelectorAll('.student-block');
  let studentsText = '';
  const studentsData = []; // structured version, for the Apps Script / Admissions pipeline submission
  panels.forEach((panel, idx) => {
    const n = idx + 1;
    const sr = document.querySelector('input[name="s'+n+'_status"]:checked');
    ...
    studentsData.push({
      status: sr ? sr.value : '', program: raw('s'+n+'_program'),
      fname: raw('s'+n+'_fname'), mname: raw('s'+n+'_mname'), lname: raw('s'+n+'_lname'),
      dob: raw('s'+n+'_dob'), age: raw('s'+n+'_age'), gender: raw('s'+n+'_gender'), grade: raw('s'+n+'_grade'),
      quran: raw('s'+n+'_quran'), hifz: raw('s'+n+'_hifz'), arabic: raw('s'+n+'_arabic'), prevSchool: raw('s'+n+'_prev_school')
    });
  });
  ...
  // ---- Dual-submit: also send to the AMAN Admissions pipeline (Google Sheet + Apps
  // Script), independently of Formspree. This is fire-and-forget and best-effort —
  // it never blocks or gates the success screen below, and if it fails silently
  // (e.g. APPS_SCRIPT_URL hasn't been configured yet) the family's application is
  // still safely captured by Formspree. See AMAN-Admissions-AppsScript.gs.
  try {
    if (APPS_SCRIPT_URL && APPS_SCRIPT_URL.indexOf('http') === 0) {
      const admParams = new URLSearchParams();
      admParams.append('application_id', ref);
      admParams.append('students_json', JSON.stringify(studentsData));
      admParams.append('parent_name', g('p_fname') + ' ' + g('p_mname') + ' ' + g('p_lname'));
      admParams.append('parent_email', g('p_email'));
      admParams.append('parent_phone', g('p_phone'));
      admParams.append('contact_pref', g('p_contact_pref'));
      admParams.append('address', g('p_street') + ', ' + g('p_city') + ', ' + g('p_state') + ' ' + g('p_zip'));
      admParams.append('emergency', g('ec_fname') + ' ' + g('ec_lname') + ' — ' + g('ec_relation') + ' — ' + g('ec_phone'));
      admParams.append('pickup', g('p_pickup'));
      admParams.append('heard', g('p_source'));
      admParams.append('interests', interests);
      admParams.append('goals', g('a_goals'));
      admParams.append('learning', g('a_learning'));
      admParams.append('health', g('h_conditions'));
      admParams.append('allergy', allergy ? allergy.value : '—');
      admParams.append('allergy_detail', g('h_allergy'));
      admParams.append('meds', g('h_meds'));
      admParams.append('waiver', document.getElementById('ag5').checked ? 'Yes' : 'No');
      admParams.append('media', document.getElementById('ag6').checked ? 'Yes' : 'No');
      admParams.append('signed_by', g('sig_fname') + ' ' + g('sig_lname'));
      admParams.append('signed_date', g('sig_date'));
      const hp = document.getElementById('hp_field');
      admParams.append('hp_field', hp ? hp.value : '');
      // mode: 'no-cors' avoids Apps Script's lack of CORS headers on POST; we can't
      // read the response, which is fine since we don't act on it either way.
      fetch(APPS_SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: admParams }).catch(() => {});
    }
  } catch (e) { /* never let this interfere with the main Formspree submission */ }
...
function showProgram(prog) {
  ...
  if(btnBro) btnBro.classList.remove('psel-active');
  if(btnSis) btnSis.classList.remove('psel-active');
  if(!isOpen) {
    const target = prog === 'brothers' ? bro : sis;
    const btn    = prog === 'brothers' ? btnBro : btnSis;
    if(target) target.style.display = 'block';
    if(btn)    btn.classList.add('psel-active');
    setTimeout(() => {
...
function submitSummerForm(){
  ...
  // ---- Dual-submit: also send to the Amaanah Admissions pipeline (same Google
  // Sheet + Apps Script as the AMAN form, writing to a separate "Amaanah
  // Applications" tab). Independent of Formspree; fire-and-forget; never blocks
  // or gates the success screen above. See AMAN-Admissions-AppsScript.gs.
  try {
    if (APPS_SCRIPT_URL && APPS_SCRIPT_URL.indexOf('http') === 0) {
      ...
      blocks.forEach((b) => {
        ...
        participantsData.push({
          fname: ins[0] ? ins[0].value : '',
          lname: ins[1] ? ins[1].value : '',
          tshirt: (b.querySelector('select') || {}).value || '',
          dob: (b.querySelector('input[type=date]') || {}).value || '',
          age: (b.querySelector('input[id^="age"]') || {}).value || '',
          health: (b.querySelector('input[placeholder^="Optional"]') || {}).value || ''
        });
      });
      const emg = document.querySelectorAll('#emergencyInfo input');
      const admParams = new URLSearchParams();
      admParams.append('program_type', 'Amaanah');
      admParams.append('application_id', ref);
      admParams.append('summer_program', prog); // 'Brothers' or 'Sisters'
      admParams.append('participants_json', JSON.stringify(participantsData));
      admParams.append('registered_by', regBy === 'self' ? 'Participant (18+)' : 'Parent/Guardian');
      if (regBy !== 'self') {
        const pi = document.getElementById('parentInfo');
        const pins = pi.querySelectorAll('input');
        admParams.append('parent_name', pins[0] ? pins[0].value : '');
        admParams.append('relationship', pins[1] ? pins[1].value : '');
        admParams.append('parent_phone', pins[2] ? pins[2].value : '');
        admParams.append('parent_email', pins[3] ? pins[3].value : '');
      }
      admParams.append('emergency_name', emg[0] ? emg[0].value : '');
      admParams.append('emergency_phone', emg[1] ? emg[1].value : '');
      admParams.append('heard', document.getElementById('hearAbout') ? document.getElementById('hearAbout').value : '');
      admParams.append('waiver', document.getElementById('agreeBoxWaiver').checked ? 'Yes' : 'No');
      admParams.append('media', document.getElementById('agreeBoxMedia').checked ? 'Yes' : 'No');
      const sigNameEl = document.getElementById('sp_sig_name');
      admParams.append('signed_by', sigNameEl ? sigNameEl.value : '');
      admParams.append('signed_date', document.getElementById('spSigDate') ? document.getElementById('spSigDate').value : '');
      // Reuses the single shared honeypot field from the AMAN form (still present
      // in the DOM — just visually hidden — regardless of which program is shown).
      const hp = document.getElementById('hp_field');
      admParams.append('hp_field', hp ? hp.value : '');
      fetch(APPS_SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: admParams }).catch(() => {});
    }
